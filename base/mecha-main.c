...
#define MECHA_ADJ_SYNTAX_ERR	"Syntax error. For help, type HELP.\n"
...
static int MechaAdjCJitter(short int argc, char* argv[]);
...
static const struct MechaDiagCommand AdjCommands[]={
	{"CJITT",		"CJITT <mode>",		"Gets jitter measurement continously every second. Modes: 1, 16, 256",					&MechaAdjCJitter},
	{"INIT",		"INIT <mode>",		"Performs initialization. Modes:\n"
										"\tCD\t- CD test mode.\n"
										"\tDVD-SL\t- DVD-SL test mode.\n"
										"\tDVD-DL\t- DVD-DL test mode.\n"
										"\tSKEW\t- Skew adjustment (DVD-SL) mode.",												&MechaAdjInit},
	{"SLED",		"SLED <mode>",		"Controls the sled. Modes:\n"
										...
										"\tTRACKING ON/OFF\t- Switches on/off tracking.",										&MechaAdjSled},
	{"PLAY",		"PLAY <mode>",		"Starts play mode Modes:\n"
										...
										"\t\t2\t- 2x mode\n"
										"\t\t3\t- 1.6x-4x mode\n"
										"\t\tFJ\t- Focus Jump (DVD-DL only)",													&MechaAdjPlay},
	...
	{"TILT",		"TILT <mode>",		"Controls the auto-tilt motor. Modes:\n"
										...
										"\tWRITE\t\t- Write new parameters",													&MechaAdjAutoTilt},
	...
	{"HELP",		"HELP",				"Displays this help message.\nType HELP <command> to get help on specific commands.\n"
										"Entering a blank line will cause the previous command entered to be executed.",		&MechaAdjHelp},
...
static const struct MechaDiagCommand TestCommands[]={
	{"DISC",		"DISC <type>",		"Sets up the circuit/disc mode:\n"
										...
										"*CAUTION: Do not stare into the laser beam!",											&MechaTestDiscControl},
	{"LASER",		"LASER <mode>",		"Controls the laser:\n"
										...
										"*CAUTION: Do not stare into the laser beam!",											&MechaTestLaserControl},
	{"SERVO",		"SERVO <mode>",		"Controls the servo circuit:\n"
										"\tAUTO*\t- Automatically adjust the servo circuit\n"
										"*NOTE: sled must be at the home position",												&MechaTestServoControl},
	{"SLED",		"SLED <mode>",		"Controls the sled. Modes:\n"
										"\tIN\t\t- Moves the sled to the inner-most position.\n"
										"\tHOME\t\t- Moves the sled to the home position.\n"
										"\tMID\t\t- Moves the sled to the middle position.\n"
	...
	{"PLAY",		"PLAY <mode>",		"Starts play mode. Modes:\n"
										...
										"\tSTOP\t- Stop playing\n"
										"\tFJ\t- Focus Jump (DVD-DL only)",														&MechaTestPlay},
	{"TRAY",		"TRAY <mode>",		"Controls the tray. Modes:\n"
										"\tOPEN*\t- Ejects the tray\n"
										"\tCLOSE*\t- Retracts the tray\n"
										"\tIN-SW\t- Displays the status of the IN-switch\n"
	...
	{"SPIND",		"SPIND <mode>",		"Controls the spindle motor\n"
										...
										"\tCLV-A\t- Enters CLV-A mode",															&MechaTestSpindle},
	{"HELP",		"HELP",				"Displays this help message.\nType HELP <command> to get help on specific commands.\n"
										"Entering a blank line will cause the previous command entered to be executed.",		&MechaTestHelp},
...
// ADDED 1.12 12/01/2021 John-John
static int MechaAdjCJitter(short int argc, char* argv[])
{
	int res;
	int rescmp;
	...
	if (argc == 2)
	{
		...
		printf("Press ESCAPE key to stop continuous jitter measurement.\n");
		do {
			res = MechaCommandExecute(MECHA_CMD_JITTER, timeout, commandArg, value, sizeof(value));
			count++;
			if (res < 0) {
				printf("\nError %d\n", res);
				done = 1;
			} else {
				rescmp = strcmp(oldValue, value);
				if (rescmp == 0) {
					trend = "EQUAL";
				}
				else {
				...
				if (strcmp(min, value) >= 0) strncpy(min, value, 8);
				if (strcmp(max, value) < 0) strncpy(max, value, 8);
				putchar('\r');
				printf("%s -> %s (%s) / MIN=%s, MAX=%s, COUNT=%i", &oldValue[1], &value[1], trend, &min[1], &max[1], count);
				strncpy(oldValue, value, 8);
				if (kbhit()) {
					done = 1;
					printf("\n");
				} else
...
static int MechaAdjGetError(short int argc, char *argv[])
{
	...
	if(!pstricmp(argv[1], "DVD"))
	{
		switch(status)
		{
			...
			case MECHA_ADJ_STATE_DVDDL_1p64:
				// JOHN-JOHN 2021 
				// 1 - MECHA_CMD_DSP_ERROR_RATE command seems to be preceeded by MECHA_CMD_DSP_ERROR_RATE_CTL (START) to work.
				MechaCommandAdd(MECHA_CMD_DSP_ERROR_RATE_CTL, "01", id++, MECHA_CMD_TAG_MECHA_DVD_ERROR_RATE, 2000, "DVD-SL DSP ERROR RATE (START)");
				if (MechaCommandExecuteList(&MechaAdjTxHandler, &MechaAdjRxHandler) != 0) {
					printf("Failed to execute MECHA_CMD_DSP_ERROR_RATE_CTL (START).\n");
					break;
				}
				// 2 - Get all the values 
				MechaCommandAdd(MECHA_CMD_DSP_ERROR_RATE,		"08", id++, MECHA_CMD_TAG_MECHA_DVD_ERROR_RATE,		2000,	"DVD-SL GET DSP PI+PO-CC RATE");
				if(MechaCommandExecuteList(&MechaAdjTxHandler, &MechaAdjRxHandler) == 0)
				{
					...
					printf("Failed to execute DVD-SL GET DSP PI+PO-CC RATE - 08.\n");
				}
				// 3 - MECHA_CMD_DSP_ERROR_RATE command seems to be succeeded by MECHA_CMD_DSP_ERROR_RATE_CTL (END). 
				MechaCommandAdd(MECHA_CMD_DSP_ERROR_RATE_CTL, "00", id++, MECHA_CMD_TAG_MECHA_DVD_ERROR_RATE, 2000, "DVD GET DSP ERROR RATE");
				if (MechaCommandExecuteList(&MechaAdjTxHandler, &MechaAdjRxHandler) != 0) {
					printf("Failed to execute MECHA_CMD_DSP_ERROR_RATE_CTL (END).\n");
					break;
				}
			default:
			...
			if(argc == 3)
			{
				...
				{	//Micro reverse
					...
					if((result = MechaCommandExecute(MECHA_CMD_SLED_CTL_MICRO, 2000, args, buffer, sizeof(buffer))) < 0 || (result = strtoul(buffer, NULL, 16)) != 0)
						printf("Error %d\n", result);
					SledIsAtHome = 0;
				} else if(!pstricmp(argv[2], "OUT")) {	//Micro forward
			...
			if(argc == 3)
			{
				...
				{	//Biphs reverse
					...
					if((result = MechaCommandExecute(MECHA_CMD_SLED_CTL_BIPHS, 2000, args, buffer, sizeof(buffer))) < 0 || (result = strtoul(buffer, NULL, 16)) != 0)
						printf("Error %d\n", result);
					SledIsAtHome = 0;
				} else if(!pstricmp(argv[2], "OUT")) {	//Biphs forward
					sprintf(args, "01%04x", StepAmount);
					if((result = MechaCommandExecute(MECHA_CMD_SLED_CTL_BIPHS, 2000, args, buffer, sizeof(buffer))) < 0 || (result = strtoul(buffer, NULL, 16)) != 0)
						printf("Error %d\n", result);
					SledIsAtHome = 0;
